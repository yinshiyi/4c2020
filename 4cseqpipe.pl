...
sub map_raw {
	...
	foreach $id (@$ids) {
		...
		system("scripts/4cmap.sh -dataidx=$index_fn -raw_dir=$rawdir -id=$id -groot=$groot -trackset=$trackset -binsize=$binsize -statfile=stats/$id.stat -max_mismatches=3 -min_precision_for_weight=0.0");
	}
}
...
sub nearcis2plot {
	...
	my($min_cutoff) = get_opt("min_cutoff",11.2);


	print STDERR "Creating near-cis plots for ID(s) $expname\n";
	system("echo tables/$expname.nearcis.norm.txt $res $feat_tab $stat $adjust_color $png_fn $color_def_script $pfrom $pto $int_type $int_down $int_up $trunc_pct $trunc_max_n $ymax $min_cutoff");
	system("$::Rscript scripts/create_nearcis_plots.r tables/$expname.nearcis.norm.txt $res $feat_tab $stat $adjust_color $png_fn $color_def_script $pfrom $pto $int_type $int_down $int_up $trunc_pct $trunc_max_n $ymax $min_cutoff");
}
